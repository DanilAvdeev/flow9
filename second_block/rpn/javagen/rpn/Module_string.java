...
final public class Module_string {
	public static final Struct f_addReverse(Struct al, String as, int ai) {
		TAIL_CALL: for(;;) {
			if ((ai<0)) {
				return al;
			} else {
				{
					final Struct l0___tmp = (new Struct_Cons(Module_string.f_getCharAt(as, ai), al));
	...
	public static final String f_reverse(String as) {
			final int l0_l = Native.strlen(as);
			if ((l0_l<=1)) {
				return as;
			} else {
				if ((l0_l<=128)) {
					return Native.list2string(Module_string.f_addReverse(Module_list.f_makeList(), as, (l0_l-1)));
				} else {
					final int l1_half = (l0_l/2);
					final String l2_left = Native.substring(as, 0, l1_half);
					final String l3_right = Native.substring(as, l1_half, l0_l);
					return (Module_string.f_reverse(l3_right)+Module_string.f_reverse(l2_left));
				}
			}
		}
